...
```{r}
install.packages(c("tidyverse", "readr", "naniar", "VIM", "recipes", "Hmisc", "psych", "skimr","knitr"))
```
...
ggplot(data = device_summary, 
       aes(x = factor(`Source...Medium`), y = total_users, fill = `Source...Medium`)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Device usage by user", x = "Device", y = "Users") +
  theme_minimal() +
  coord_flip()
...
ggplot(data = device_summary, 
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Device usage by new user", x = "Device", y = "New Users") +
  theme_minimal() +
  coord_flip()
